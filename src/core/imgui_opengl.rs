/// Modified version of [rust-imgui-opengl-renderer](https://github.com/eliaperantoni/rust-imgui-opengl-renderer).
...
impl Renderer {
    ...
    ) -> Self
    {
      ...
      \0";
            ...
            let vert_sources = [glsl_version.as_ptr().cast(), vert_source.as_ptr().cast()];
            let vert_sources_len = [glsl_version.len() as GLint - 1, vert_source.len() as GLint - 1];
            let frag_sources = [glsl_version.as_ptr().cast(), frag_source.as_ptr().cast()];
            ...
            let locs = Locs {
                texture: gl::GetUniformLocation(program, c"Texture".as_ptr().cast()),
                proj_mtx: gl::GetUniformLocation(program, c"ProjMtx".as_ptr().cast()),
                position: gl::GetAttribLocation(program, c"Position".as_ptr().cast()) as _,
                uv: gl::GetAttribLocation(program, c"UV".as_ptr().cast()) as _,
                color: gl::GetAttribLocation(program, c"Color".as_ptr().cast()) as _,
            };
...

            {
                ...
                gl::TexImage2D(gl::TEXTURE_2D, 0, gl::RGBA as _, texture.width as _, texture.height as _, 0, gl::RGBA, gl::UNSIGNED_BYTE, texture.data.as_ptr().cast());
    ...
    pub fn render(
        ...
        unsafe {
            ...
            let [width, height] = context.io().display_size;
            let [scale_w, scale_h] = context.io().display_framebuffer_scale;
            ...
            gl::UniformMatrix4fv(self.locs.proj_mtx, 1, gl::FALSE, matrix.as_ptr().cast());
            gl::BindSampler(0, 0);
            ...
            if draw_data.draw_lists_count() != 0 {
                for draw_list in draw_data.draw_lists() {
                    ...
                    gl::BindBuffer(gl::ARRAY_BUFFER, self.vbo);
                    gl::BufferData(gl::ARRAY_BUFFER, mem::size_of_val(vtx_buffer) as GLsizeiptr, vtx_buffer.as_ptr().cast(), gl::STREAM_DRAW);

                    gl::BindBuffer(gl::ELEMENT_ARRAY_BUFFER, self.ebo);
                    gl::BufferData(gl::ELEMENT_ARRAY_BUFFER, mem::size_of_val(idx_buffer) as GLsizeiptr, idx_buffer.as_ptr().cast(), gl::STREAM_DRAW);
                        ...
                        match cmd {
                            DrawCmd::Elements {
                                ...
                                gl::Scissor((x * scale_w) as GLint,
                                            w.mul_add(-scale_h, fb_height) as GLint,
...
fn field_offset<T, U, F: for<'a> FnOnce(&'a T) -> &'a U>(f: F) -> usize {
    unsafe {
        ...
        let offset = {
            ...
            std::ptr::from_ref(field) as usize - std::ptr::from_ref(&instance) as usize
        };
